...
pacman::p_load(BiocManager, # Colorblind friendly pallettes
               ...
               visdat,
               colorblindr,
               mice) # Another EDA visualization package
...
na.dataset |>
  select(X, airline,	avail_seat_km_per_week,	year_range, type_of_event,	n_events) |>
...
dataset %>%
  ggplot(aes(x = airline, y = n_events, fill = n_events)) +
...
rpart_na_imp_events <- na.dataset |>
  imputate_na(n_events, method = "rpart")
...
Note: My dataset didn't have any NA values, hence rpart cannot be executed well.
...
mice_na_imp_events <- na.dataset |>
  imputate_na(n_events, method = "mice", seed = 123)
```
